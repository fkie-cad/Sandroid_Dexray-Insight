...
from tests.fixtures.real_apk_fixtures import TEST_RESULTS_CACHE_DIR  # noqa: F401
from tests.fixtures.real_apk_fixtures import available_sample_apks  # noqa: F401
from tests.fixtures.real_apk_fixtures import cached_baseline_results  # noqa: F401
from tests.fixtures.real_apk_fixtures import ci_safe_apk_path  # noqa: F401
from tests.fixtures.real_apk_fixtures import is_ci_environment  # noqa: F401
from tests.fixtures.real_apk_fixtures import mock_external_apis  # noqa: F401
from tests.fixtures.real_apk_fixtures import real_apk_test_config  # noqa: F401
from tests.fixtures.real_apk_fixtures import save_baseline_results  # noqa: F401
...
class RegressionAnalyzer:
    ...
    def normalize_result_for_comparison(result_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        for module in modules_to_normalize:
            if module in normalized:
                normalized[module] = RegressionAnalyzer._normalize_module_result(normalized[module], module)
    ...
    def find_differences(current: dict[str, Any], baseline: dict[str, Any]) -> list[str]:
        ...
        for key in common_keys:
            key_diffs = RegressionAnalyzer._compare_values(norm_current[key], norm_baseline[key], f"root.{key}")
    ...
    def _compare_values(current: Any, baseline: Any, path: str) -> list[str]:
        ...
        if isinstance(current, dict) and isinstance(baseline, dict):
            ...
            for key in current_keys & baseline_keys:
                sub_diffs = RegressionAnalyzer._compare_values(current[key], baseline[key], f"{path}.{key}")
            ...
            for i in range(min_len):
                elem_diffs = RegressionAnalyzer._compare_values(current[i], baseline[i], f"{path}[{i}]")
...
class TestRealAPKRegression:
    ...
    def test_ci_safe_apk_baseline_regression(
        self, ci_safe_apk_path, real_apk_test_config, cached_baseline_results, mock_external_apis
    ):
        """
        ...
        critical_differences = [
            diff for diff in differences if not any(acceptable in diff for acceptable in acceptable_difference_types)
        ]

        if critical_differences:
            pytest.fail(f"Critical regression detected with {len(critical_differences)} significant differences")

    def test_module_specific_regression_library_detection(
        self, ci_safe_apk_path, real_apk_test_config, mock_external_apis
    ):
        """
        ...
        if cache_file.exists():
            ...
            baseline_lib_names = {
                lib.get("name") for lib in baseline_lib_detection.get("detected_libraries", []) if isinstance(lib, dict)
            }
    ...
    def test_security_assessment_regression(self, ci_safe_apk_path, real_apk_test_config, mock_external_apis):
        ...
        ), "Clean APK should have low risk score (≤50)"  # Reasonable threshold for clean APK
        ...
        if cache_file.exists():
            ...
            assert (
                risk_variance <= 10
            ), f"Risk score variance too high for clean APK: {baseline_risk_score} -> {current_risk_score}"

            # Finding count should be relatively stable for clean APK
            finding_count_diff = abs(len(current_findings) - len(baseline_findings))
            assert (
                finding_count_diff <= 2
            ), f"Finding count changed significantly for clean APK: {len(baseline_findings)} -> {len(current_findings)}"
            ...
            print(
                f"Saved clean APK security assessment baseline: {current_risk_score} risk, {len(current_findings)} findings"
            )
    ...
    def test_performance_regression(self, ci_safe_apk_path, real_apk_test_config, mock_external_apis):
        ...
        if perf_cache_file.exists():
            with open(perf_cache_file) as f:
            ...
            if baseline_avg > 0:
                ...
                assert (
                    performance_ratio <= max_acceptable_ratio
    ...
    def test_multiple_apk_regression(
        self, available_sample_apks, real_apk_test_config, is_ci_environment, mock_external_apis
    ):
        """
...
class TestRegressionUtilities:
    ...
    def test_regression_analyzer_normalization(self):
        ...
        test_result = {
            ...
            "apk_overview": {"package_name": "com.test.app", "app_name": "Test App"},
            "library_detection": {
                "detected_libraries": [{"name": "LibraryB", "version": "2.0"}, {"name": "LibraryA", "version": "1.0"}],
    ...
    def test_regression_hash_consistency(self):
        ...
        test_result = {"apk_overview": {"package_name": "com.test.app"}, "metadata": {"version": "1.0"}}
