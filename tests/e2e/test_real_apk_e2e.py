...
class TestRealAPKEndToEnd:
    ...
    def test_cli_basic_analysis_ci_safe(self, ci_safe_apk_path, tmp_path, mock_external_apis):
        ...
        result = run_cli_command(
            [str(ci_safe_apk_path)], capture_output=True, text=True, timeout=300, cwd=str(tmp_path)
        )
    ...
    def test_cli_with_security_assessment(self, ci_safe_apk_path, tmp_path, mock_external_apis):
        ...
        """
        result = run_cli_command(
            [str(ci_safe_apk_path), "-s"],  # Enable security assessment (--security not supported, use -s)
            capture_output=True,
            text=True,
            timeout=300,
            cwd=str(tmp_path),
    ...
    def test_cli_with_custom_config(self, ci_safe_apk_path, tmp_path, real_apk_test_config, mock_external_apis):
        ...
        result = run_cli_command(
            [str(ci_safe_apk_path), "--config", str(config_file)],
            capture_output=True,
            text=True,
            timeout=300,
            cwd=str(tmp_path),
    ...
    def test_cli_console_output_format(self, ci_safe_apk_path, mock_external_apis):
        ...
        result = run_cli_command([str(ci_safe_apk_path)], capture_output=True, text=True, timeout=300)
        ...
        assert "Package:" in console_output or "package" in console_output.lower(), "Should display package information"
    ...
    def test_cli_error_handling_invalid_apk(self, tmp_path, mock_external_apis):
        ...
        result = run_cli_command([str(invalid_apk)], capture_output=True, text=True, timeout=60, cwd=str(tmp_path))
    ...
    def test_cli_nonexistent_file(self, mock_external_apis):
        ...
        """
        result = run_cli_command(["/nonexistent/path/fake.apk"], capture_output=True, text=True, timeout=30)
    ...
    def test_cli_performance_tracking(self, ci_safe_apk_path, tmp_path, performance_tracker, mock_external_apis):
        ...
        result = run_cli_command(
            [str(ci_safe_apk_path), "--debug", "INFO"],  # Enable some logging for performance tracking
            capture_output=True,
            text=True,
            timeout=300,
            cwd=str(tmp_path),
...
class TestRealAPKE2ELocalDevelopment:
    ...
    def test_cli_multiple_apks_batch_analysis(self, available_sample_apks, tmp_path, mock_external_apis):
        ...
        for apk_path in test_apks:
            ...
            cmd = [sys.executable, "-m", "dexray_insight.asam", str(apk_path)]

            try:
                start_time = time.time()
                result = subprocess.run(
                    cmd, capture_output=True, text=True, timeout=300, env=env, cwd=str(apk_output_dir)
                )
                end_time = time.time()
    ...
    def test_cli_comprehensive_analysis_large_sample(self, available_sample_apks, tmp_path, mock_external_apis):
        ...
        large_apks = [apk for apk in available_sample_apks if apk.stat().st_size < 50 * 1024 * 1024]  # Under 50MB
