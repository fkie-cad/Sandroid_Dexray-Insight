...
from dexray_insight.results.apkidResults import ApkidFileAnalysis
from dexray_insight.results.apkidResults import ApkidResults
...
class TestFullAnalysisResultsPrintSummaryHeader:
    ...
    def test_print_summary_header_displays_formatted_header(self):
        ...
        # Assert
        expected_lines = ["\n" + "=" * 80, "📱 DEXRAY INSIGHT ANALYSIS SUMMARY", "=" * 80]
...
class TestFullAnalysisResultsPrintSecurityAssessmentSummary:
    ...
    def test_print_security_assessment_summary_displays_risk_metrics(self):
        ...
        results.security_assessment = {
            ...
            "findings_by_severity": {"critical": 2, "high": 5, "medium": 6, "low": 2},
...
class TestFullAnalysisResultsPrintToolAnalysisSummary:
    ...
    def test_print_tool_analysis_summary_displays_apkid_compiler_info(self):
        ...
        results.apkid_analysis.files = [
            ApkidFileAnalysis(filename="classes.dex", matches={"compiler": ["dx"], "packer": []})
        ]
    ...
    def test_print_tool_analysis_summary_warns_about_repacking(self):
        ...
        results.apkid_analysis.files = [
            ApkidFileAnalysis(
                ...
                matches={"compiler": ["dexlib"], "packer": []},  # Repacking indicator
            )
        ]
...
class TestFullAnalysisResultsRefactoredPrintAnalystSummary:
    ...
    def test_refactored_print_analyst_summary_calls_all_section_functions(self):
        ...
        with patch.object(results, "_print_summary_header") as mock_header, patch.object(
            results, "_print_apk_information"
        ) as mock_apk, patch.object(results, "_print_permissions_summary") as mock_perms, patch.object(
            results, "_print_string_analysis_summary"
        ) as mock_strings, patch.object(results, "_print_security_assessment_summary") as mock_security, patch.object(
            results, "_print_tool_analysis_summary"
        ) as mock_tools, patch.object(results, "_print_component_behavior_summary") as mock_components, patch.object(
            results, "_print_summary_footer"
        ) as mock_footer:
    ...
    def test_refactored_print_analyst_summary_maintains_output_compatibility(self):
        ...
        results.apk_overview.permissions = {"permissions": ["android.permission.INTERNET"]}
        results.in_depth_analysis = Results()
