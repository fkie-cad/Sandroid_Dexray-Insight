...
class TestManifestAnalysisPermissionAnalysis:
    ...
    def test_analyze_custom_permissions_processes_protection_levels(self):
        ...
        # Arrange
        permission1 = create_mock_manifest_element(
            "permission",
            {"android:name": "com.test.CUSTOM_PERMISSION", "android:protectionLevel": "0x00000001"},  # dangerous
        )
        permission2 = create_mock_manifest_element(
            "permission",
            {"android:name": "com.test.NORMAL_PERMISSION", "android:protectionLevel": "0x00000000"},  # normal
        )
        ...
        with patch("dexray_insight.apk_overview.manifest_analysis._analyze_custom_permissions") as mock_func:
            expected_result = {"com.test.CUSTOM_PERMISSION": "dangerous", "com.test.NORMAL_PERMISSION": "normal"}
    ...
    def test_analyze_custom_permissions_handles_missing_protection_level(self):
        ...
        # Arrange
        permission = create_mock_manifest_element(
            "permission",
            {
...
class TestManifestAnalysisSDKVersionValidation:
    ...
    def test_validate_sdk_versions_identifies_vulnerable_versions(self):
        ...
        # Arrange
        man_data_dic = {"min_sdk": "16", "target_sdk": "30"}  # Android 4.1 - vulnerable

        # Act - This will fail initially (RED phase)
        with patch("dexray_insight.apk_overview.manifest_analysis._validate_sdk_versions") as mock_func:
            expected_findings = [("vulnerable_os_version", ("4.1-4.1.2", "16"), ())]
    ...
    def test_validate_sdk_versions_handles_secure_versions(self):
        ...
        # Arrange
        man_data_dic = {"min_sdk": "30", "target_sdk": "33"}  # Android 11 - secure
...
class TestManifestAnalysisApplicationConfiguration:
    ...
    def test_analyze_application_configuration_detects_security_issues(self):
        ...
        # Arrange
        application = create_mock_manifest_element(
            "application",
            {
    ...
    def test_analyze_application_configuration_handles_secure_settings(self):
        ...
        application = create_mock_manifest_element(
            "application",
            {"android:allowBackup": "false", "android:debuggable": "false", "android:usesCleartextTraffic": "false"},
        )
...
class TestManifestAnalysisComponentAnalysis:
    ...
    def test_analyze_components_processes_activities(self):
        ...
        # Arrange
        activity = create_mock_manifest_element(
    ...
    def test_analyze_components_processes_services(self):
        ...
        # Arrange
        service = create_mock_manifest_element(
            "service",
            {
...
class TestManifestAnalysisExportStatusAnalysis:
    ...
    def test_determine_export_status_identifies_explicitly_exported(self):
        ...
        # Arrange
        component = create_mock_manifest_element(
...
class TestManifestAnalysisPermissionProtectionAnalysis:
    ...
    def test_analyze_permission_protection_validates_signature_protection(self):
        ...
        with patch("dexray_insight.apk_overview.manifest_analysis._analyze_permission_protection") as mock_func:
            expected_result = {"protection_level": "signature", "is_secure": True, "risk_assessment": "low"}
            mock_func.return_value = expected_result
    ...
    def test_analyze_permission_protection_flags_dangerous_permissions(self):
        ...
        # Act
        with patch("dexray_insight.apk_overview.manifest_analysis._analyze_permission_protection") as mock_func:
            expected_result = {"protection_level": "dangerous", "is_secure": False, "risk_assessment": "high"}
            mock_func.return_value = expected_result
...
class TestManifestAnalysisDataTagAnalysis:
    ...
    def test_analyze_data_tags_detects_secret_codes(self):
        ...
        # Arrange
        data_tag = create_mock_manifest_element(
        ...
        with patch("dexray_insight.apk_overview.manifest_analysis._analyze_data_tags") as mock_func:
            expected_findings = [("dialer_code_found", ("*#*#123456#*#*",), ())]
            mock_func.return_value = expected_findings
    ...
    def test_analyze_data_tags_detects_sms_ports(self):
        ...
        # Arrange
        data_tag = create_mock_manifest_element("data", {"android:port": "8080"})
        ...
        # Act
        with patch("dexray_insight.apk_overview.manifest_analysis._analyze_data_tags") as mock_func:
            expected_findings = [("sms_receiver_port_found", ("8080",), ())]
            mock_func.return_value = expected_findings
...
class TestManifestAnalysisIntentPriorityAnalysis:
    ...
    def test_analyze_intent_priorities_detects_high_intent_priority(self):
        ...
        # Arrange
        intent_filter = create_mock_manifest_element("intent-filter", {"android:priority": "1000"})  # Suspiciously high

        action = create_mock_manifest_element("action", {"android:priority": "500"})  # Also high

        mfxml = create_mock_manifest_xml({"intent-filter": [intent_filter], "action": [action]})
        ns = "android"
...
class TestManifestAnalysisGrantUriPermissionAnalysis:
    ...
    def test_analyze_grant_uri_permissions_detects_improper_permissions(self):
        ...
        # Arrange
        grant_uri1 = create_mock_manifest_element("grant-uri-permission", {"android:pathPrefix": "/"})  # Too permissive

        grant_uri2 = create_mock_manifest_element(
...
class TestManifestAnalysisResultProcessing:
    ...
    def test_process_analysis_results_formats_findings(self):
        ...
        # Act - This will fail initially (RED phase)
        with patch("dexray_insight.apk_overview.manifest_analysis._process_analysis_results") as mock_func, patch(
            "dexray_insight.apk_overview.manifest_analysis.MANIFEST_DESC", mock_manifest_desc
        ):
            expected_result = [
                {
...
class TestManifestAnalysisRefactoredMain:
    ...
    def test_refactored_manifest_analysis_calls_all_analysis_functions(self):
        ...
        with patch(
            "dexray_insight.apk_overview.manifest_analysis._analyze_custom_permissions"
        ) as mock_permissions, patch(
            "dexray_insight.apk_overview.manifest_analysis._validate_sdk_versions"
        ) as mock_sdk, patch(
            "dexray_insight.apk_overview.manifest_analysis._analyze_application_configuration"
        ) as mock_app_config, patch(
            "dexray_insight.apk_overview.manifest_analysis._analyze_components"
        ) as mock_components, patch(
            "dexray_insight.apk_overview.manifest_analysis._analyze_grant_uri_permissions"
        ) as mock_grant_uri, patch(
            "dexray_insight.apk_overview.manifest_analysis._analyze_data_tags"
        ) as mock_data_tags, patch(
            "dexray_insight.apk_overview.manifest_analysis._analyze_intent_priorities"
        ) as mock_intent_priorities, patch(
            "dexray_insight.apk_overview.manifest_analysis._process_analysis_results"
        ) as mock_process_results, patch(
            "dexray_insight.apk_overview.manifest_analysis._integrate_network_security"
        ) as mock_network_security:
...
class TestExistingManifestAnalysisFunctions:
    ...
    def test_get_browsable_activities_functionality(self):
        ...
        intent_filter.getElementsByTagName.side_effect = lambda tag: {"action": [action], "category": [category]}.get(
            tag, []
        )
