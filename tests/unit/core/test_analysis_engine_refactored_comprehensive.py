...
class TestAnalysisEngineBuildApkOverview:
    ...
    def test_build_apk_overview_with_successful_result(self, engine, mock_successful_apk_overview_result):
        ...
        with patch("dexray_insight.results.apkOverviewResults.APKOverview") as mock_overview_class:
            ...
            assert (
                mock_overview.cross_platform_framework == mock_successful_apk_overview_result.cross_platform_framework
            )

    def test_build_apk_overview_with_failed_result_uses_fallback(
        self, engine, mock_failed_apk_overview_result, mock_manifest_result
    ):
        """Test APK overview building falls back to manifest analysis when APK overview fails."""
        # Arrange
        module_results = {"apk_overview": mock_failed_apk_overview_result, "manifest_analysis": mock_manifest_result}
...
class TestAnalysisEngineBuildInDepthAnalysis:
    ...
    def test_build_in_depth_analysis_calls_all_mapping_methods(self, engine, mock_context):
        ...
        # Act
        with patch("dexray_insight.results.InDepthAnalysisResults.Results") as mock_results_class, patch.object(
            engine, "_map_manifest_results"
        ) as mock_map_manifest, patch.object(engine, "_map_permission_results") as mock_map_permission, patch.object(
            engine, "_map_signature_results"
        ) as mock_map_signature, patch.object(engine, "_map_string_results") as mock_map_string, patch.object(
            engine, "_map_library_results"
        ) as mock_map_library, patch.object(engine, "_map_tracker_results") as mock_map_tracker, patch.object(
            engine, "_map_behavior_results"
        ) as mock_map_behavior:
            mock_results = Mock()
...
class TestAnalysisEngineMapStringResults:
    ...
    def test_map_string_results_with_successful_analysis(
        self, engine, mock_in_depth_analysis, mock_successful_string_result
    ):
    ...
    def test_map_string_results_with_failed_analysis_uses_fallback(
        self, engine, mock_in_depth_analysis, mock_failed_string_result, mock_context
    ):
...
class TestAnalysisEngineBuildToolResults:
    ...
    def test_build_tool_results_with_successful_tools(self, engine):
        ...
        # Arrange
        tool_results = {
            "apkid": {"success": True, "results": {"compiler": "dx", "obfuscator": "none"}},
            "kavanoz": {"success": True, "results": {"unpacked": True, "files": ["classes.dex"]}},
        }

        # Act
        with patch("dexray_insight.results.apkidResults.ApkidResults") as mock_apkid_class, patch(
            "dexray_insight.results.kavanozResults.KavanozResults"
        ) as mock_kavanoz_class:
            mock_apkid = Mock()
            mock_kavanoz = Mock()
    ...
    def test_build_tool_results_with_failed_tools(self, engine):
        ...
        # Arrange
        tool_results = {"apkid": {"success": False}, "kavanoz": {"success": False}}
...
class TestAnalysisEngineCreateFullResultsIntegration:
    ...
    def test_create_full_results_integration(
        self, engine, mock_module_results, mock_tool_results, mock_security_results, mock_context
    ):
        """Test complete _create_full_results method integration."""
        # Act
        with patch.object(engine, "_build_apk_overview") as mock_build_overview, patch.object(
            engine, "_build_in_depth_analysis"
        ) as mock_build_in_depth, patch.object(engine, "_build_tool_results") as mock_build_tools, patch(
            "dexray_insight.results.FullAnalysisResults.FullAnalysisResults"
        ) as mock_full_results_class:
            # Set up mocks
            mock_overview = Mock()
            mock_in_depth = Mock()
            mock_apkid = Mock()
            ...
            result = engine._create_full_results(
                mock_module_results, mock_tool_results, mock_security_results, mock_context
            )
    ...
    def test_create_full_results_handles_none_security_results(
        self, engine, mock_module_results, mock_tool_results, mock_context
    ):
