...
class TestIPAddressFalsePositiveFiltering:
    ...
    def test_random_strings_filtered_out(self):
        ...
        random_strings = ["E::TB;>", "E::TT;>(", "A::BC<>", "123::xyz"]
...
class TestDomainFalsePositiveFiltering:
    ...
    def test_valid_domains_still_detected(self):
        ...
        valid_domains = ["google.com", "github.com", "stackoverflow.com", "api.example.org", "cdn.jsdelivr.net"]
        ...
        assert len(result) == len(
            valid_domains
        ), f"Valid domains not detected: missing {set(valid_domains) - set(result)}"
...
class TestAndroidPropertiesPatternMatching:
    ...
    def test_vendor_properties_detected(self):
        ...
        assert len(found_props) == len(
            vendor_props
        ), f"Vendor properties not detected: {set(vendor_props) - set(found_props.keys())}"
    ...
    def test_kotlin_properties_detected(self):
        ...
        assert len(found_props) == len(
            kotlin_props
        ), f"Kotlin properties not detected: {set(kotlin_props) - set(found_props.keys())}"
    ...
    def test_firebase_properties_detected(self):
        ...
        assert len(found_props) == len(
            firebase_props
        ), f"Firebase properties not detected: {set(firebase_props) - set(found_props.keys())}"
