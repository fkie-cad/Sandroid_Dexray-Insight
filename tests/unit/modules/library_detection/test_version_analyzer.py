...
from src.dexray_insight.modules.library_detection.utils.version_analyzer import VersionAnalysisResult
from src.dexray_insight.modules.library_detection.utils.version_analyzer import VersionAnalyzer
from src.dexray_insight.modules.library_detection.utils.version_analyzer import VersionInfo
...
class TestVersionAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = {"version_analysis": {"enabled": True, "api_timeout": 5, "cache_duration_hours": 24}}
    ...
    def test_calculate_years_behind_with_date(self):
        ...
        years_behind = self.analyzer._calculate_years_behind(current_version, latest_version, release_date)
    ...
    def test_format_version_output(self):
        ...
        analysis = VersionAnalysisResult(
            current_version="2.8.5", latest_version="2.10.1", years_behind=1.5, security_risk="MEDIUM"
        )

        formatted = self.analyzer.format_version_output("Gson", analysis, "/com/google/gson/")
    ...
    def test_check_maven_central_success(self, mock_get):
        ...
        mock_response.json.return_value = {
            "response": {"docs": [{"latestVersion": "2.10.1", "timestamp": 1609459200000}]}  # 2021-01-01
        }
    ...
    def test_get_known_google_versions(self):
        ...
        version_info = self.analyzer._get_known_google_versions("com.google.android.gms", "play-services-cast")
        self.assertIsNotNone(version_info)
        self.assertEqual(version_info.version, "21.6.0")
        self.assertTrue(version_info.is_latest)

        # Test known Firebase library
        version_info = self.analyzer._get_known_google_versions("com.google.firebase", "firebase-messaging")
        self.assertIsNotNone(version_info)
        self.assertEqual(version_info.version, "24.1.0")

        # Test unknown library
        version_info = self.analyzer._get_known_google_versions("com.google.android.gms", "unknown-service")
        self.assertIsNone(version_info)
    ...
    def test_security_only_false_always_runs(self):
        ...
        config = {"version_analysis": {"enabled": True, "security_analysis_only": False}}
        analyzer = VersionAnalyzer(config, security_analysis_enabled=False)
...
class TestVersionAnalysisResult(unittest.TestCase):
    ...
    def test_custom_analysis_date(self):
        ...
        result = VersionAnalysisResult(current_version="1.0.0", analysis_date=custom_date)
