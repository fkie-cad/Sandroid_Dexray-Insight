...
from src.dexray_insight.modules.library_detection.utils.version_analyzer import VersionAnalyzer
from src.dexray_insight.modules.library_detection.utils.version_analyzer import get_version_analyzer
from src.dexray_insight.results.LibraryDetectionResults import DetectedLibrary
from src.dexray_insight.results.LibraryDetectionResults import LibraryCategory
from src.dexray_insight.results.LibraryDetectionResults import LibraryDetectionMethod
from src.dexray_insight.results.LibraryDetectionResults import LibrarySource
from src.dexray_insight.results.LibraryDetectionResults import RiskLevel
...
class TestSecurityOnlyVersionAnalysis(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.context_with_security = AnalysisContext(
            "test.apk",
            {
                ...
                "modules": {
                    "library_detection": {"version_analysis": {"enabled": True, "security_analysis_only": True}}
                },
            },
        )
    ...
    def test_version_analyzer_with_security_analysis_enabled(self):
        ...
        analyzer = VersionAnalyzer(config=self.base_config, security_analysis_enabled=True)
    ...
    def test_version_analyzer_with_security_analysis_disabled(self):
        ...
        analyzer = VersionAnalyzer(config=self.base_config, security_analysis_enabled=False)
    ...
    def test_version_analyzer_always_enabled_mode(self):
        ...
        analyzer = VersionAnalyzer(config=config, security_analysis_enabled=False)  # Even without security analysis
    ...
    def test_version_analysis_disabled_in_config(self):
        ...
        analyzer = VersionAnalyzer(config=config, security_analysis_enabled=True)  # Even with security analysis
    ...
    def test_coordinator_version_analysis_disabled_in_config(self):
        ...
        context_disabled = AnalysisContext(
            "test.apk",
            {
                ...
                "modules": {
                    "library_detection": {
                        "version_analysis": {"enabled": False, "security_analysis_only": True}  # Disabled
                    }
                },
            },
        )
    ...
    def test_coordinator_version_analysis_always_mode(self):
        ...
        context_always = AnalysisContext(
            "test.apk",
            {
                ...
                "modules": {
                    "library_detection": {
                        "version_analysis": {"enabled": True, "security_analysis_only": False}  # Should run always
                    }
                },
            },
        )
        ...
        with patch("builtins.print") as mock_print:
            ...
            version_analysis_printed = any("LIBRARY VERSION ANALYSIS" in str(call) for call in print_calls)
            self.assertTrue(
                version_analysis_printed, "Version analysis should run when security_analysis_only is False"
            )
    ...
    def test_version_analyzer_priority_logic(self):
        ...
        config_disabled = {
            "version_analysis": {"enabled": False, "security_analysis_only": False}  # This should be ignored
        }
        ...
        config_security_only = {"version_analysis": {"enabled": True, "security_analysis_only": True}}
        ...
        config_always = {"version_analysis": {"enabled": True, "security_analysis_only": False}}
...
class TestSecurityOnlyVersionAnalysisIntegration(unittest.TestCase):
    ...
    def test_apktool_detection_engine_security_context(self):
        ...
        config = {
            "apktool_detection": {"enable_properties_detection": True},
            "version_analysis": {"enabled": True, "security_analysis_only": True},
        }
        ...
        with patch.object(engine, "_scan_lib_patterns", return_value=[]), patch.object(
            engine, "_scan_properties", return_value=[]
        ), patch.object(engine, "_scan_buildconfig_smali", return_value=[]), patch.object(
            engine, "_deduplicate_libraries", side_effect=lambda x: x
        ):
            errors = []
