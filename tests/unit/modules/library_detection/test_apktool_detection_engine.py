...
class TestApktoolDetectionEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_libinfo = [
            {"id": "test_library_1", "details": "A test utility library", "anti": [], "license": "MIT"},
            {
                "id": "test_library_2",
    ...
    def test_deduplicate_libraries(self):
        ...
        lib1 = DetectedLibrary(
            name="Test Library", package_name="com.test.lib", confidence=0.8, evidence=["Evidence 1"]
        )

        lib2 = DetectedLibrary(
            name="Test Library", package_name="com.test.lib", confidence=0.9, evidence=["Evidence 2"]
        )

        lib3 = DetectedLibrary(
            name="Different Library", package_name="com.different.lib", confidence=0.7, evidence=["Evidence 3"]
        )
...
class TestApktoolDetectionEngineIntegration(unittest.TestCase):
    ...
    def test_detect_libraries_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(buildconfig_file, "w") as f:
                f.write(
                    """.class public final Lcom/integration/test/BuildConfig;
