...
from dexray_insight.modules.native.library_version_detection import NativeLibraryVersionModule
...
class TestNativeLibraryVersionModule:
    ...
    def config(self):
        ...
        return {"enabled": True, "min_confidence": 0.6, "max_libraries_per_binary": 10}
    ...
    def test_detect_prefix_libraries_ffmpeg(self, version_module, binary_info):
        ...
        expected_results = [("FFmpeg", "n4.1.3"), ("FFmpeg", "4.2.2"), ("FFmpeg", "n3.4.8")]

        for test_string, (expected_name, expected_version) in zip(test_strings, expected_results, strict=False):
    ...
    def test_detect_prefix_libraries_openssl(self, version_module, binary_info):
        ...
        test_strings = ["--prefix=/usr/local/openssl-1.1.1", "--prefix=/build/OpenSSL-1.0.2/output"]

        expected_results = [("OpenSSL", "1.1.1"), ("OpenSSL", "1.0.2")]
    ...
    def test_detect_prefix_libraries_no_match(self, version_module, binary_info):
        ...
        test_strings = ["--prefix=/usr/local/bin", "--enable-shared", "random string with no library info"]
    ...
    def test_detect_version_strings_ffmpeg(self, version_module, binary_info):
        ...
        test_strings = ["FFmpeg version n4.1.3", "ffmpeg version 4.2.2", "FFmpeg-n3.4.8 Copyright"]

        expected_versions = ["n4.1.3", "4.2.2", "n3.4.8"]

        for test_string, expected_version in zip(test_strings, expected_versions, strict=False):
    ...
    def test_detect_version_strings_openssl(self, version_module, binary_info):
        ...
        test_strings = ["OpenSSL 1.1.1", "openssl/1.0.2"]
