...
class TestCLIIntegration:
    ...
    def test_security_flag_enables_version_analysis(self, sample_apk):
        ...
        # Run with -s flag
        result = subprocess.run(["dexray-insight", "-s", sample_apk], capture_output=True, text=True, timeout=300)
        ...
        assert result.stdout.index("📚 LIBRARY VERSION ANALYSIS") > result.stdout.index(
    ...
    def test_without_security_flag_skips_version_analysis(self, sample_apk):
        ...
        result = subprocess.run(["dexray-insight", sample_apk], capture_output=True, text=True, timeout=300)

        # Should not have version analysis section
        assert "📚 LIBRARY VERSION ANALYSIS" not in result.stdout
        assert (
            "VERSION ANALYSIS SKIPPED" in result.stdout
            or "version analysis only runs during security analysis" in result.stdout.lower()
        )
...
"""
        ...
        try:
            ...
            result = subprocess.run(
                ["dexray-insight", "-c", config_path, "-s", sample_apk], capture_output=True, text=True, timeout=300
            )
    ...
    def test_version_analysis_content_format(self, sample_apk):
        ...
        if "📚 LIBRARY VERSION ANALYSIS" in output:
            ...
            if "libraries analyzed" in version_section and "0 libraries analyzed" not in version_section:
                assert (
                    has_risk_categories
                ), f"Version analysis section should have risk categories. Content: {version_section[:500]}"
...
class TestConfigurationIntegration:
        ...
        if config_path.exists():
            ...
            for line in owasp_lines:
                if not line.strip().startswith("#"):
                    ...
                    assert (
                        "false" not in line.lower()
                    ), f"Found uncommented enable_owasp_assessment: false in dexray.yaml: {line}"
    ...
    def test_cli_args_parsing(self):
        ...
        from dexray_insight.asam import create_configuration_from_args
        from dexray_insight.asam import parse_arguments
