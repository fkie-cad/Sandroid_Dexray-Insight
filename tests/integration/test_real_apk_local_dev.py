...
class TestRealAPKLocalDevelopment:
    ...
    def test_all_available_samples_basic_analysis(
        self, available_sample_apks, real_apk_test_config, mock_external_apis
    ):
        """
            ...
            try:
                ...
                if results is not None:
                    ...
                    summary = {
                        ...
                        "libraries_detected": len(
                            results_dict.get("library_detection", {}).get("detected_libraries", [])
                        ),
    ...
    def test_malware_detection_capabilities(self, available_sample_apks, real_apk_test_config, mock_external_apis):
        """
        ...
        if malware_results:
            ...
            assert len(samples_with_findings) > 0, "At least one malware sample should trigger security findings"

    @pytest.mark.performance
    def test_performance_with_various_apk_sizes(
        self, available_sample_apks, real_apk_test_config, performance_tracker, mock_external_apis
    ):
        """
        ...
        if performance_results:
            ...
            if successful_results:
                ...
                assert (
                    slowest["duration_seconds"] < max_reasonable_duration
            ...
            try:
                ...
                if results is not None:
                    ...
                    characteristics = {
                        ...
                        "large_string_count": len(results_dict.get("string_analysis", {}).get("all_strings", []))
                        > 10000,
                        "many_permissions": len(results_dict.get("permission_analysis", {}).get("all_permissions", []))
                        > 50,
    ...
    def test_comprehensive_security_assessment(self, available_sample_apks, real_apk_test_config, mock_external_apis):
        """
        ...
        if security_summary["total_samples"] > 0:
            ...
            if security_summary["potentially_malicious_samples"] > 0:
                ...
                assert (
                    security_summary["total_findings"] > 0
...
class TestAdvancedRealAPKScenarios:
    ...
    def test_large_apk_handling(self, available_sample_apks, real_apk_test_config, mock_external_apis):
        ...
        large_apks = [apk for apk in available_sample_apks if apk.stat().st_size > 10 * 1024 * 1024]
    ...
    def test_timeout_handling(self, available_sample_apks, mock_external_apis):
        ...
        try:
            ...
            assert "timeout" in str(e).lower() or "time" in str(e).lower(), f"Should be timeout-related error: {e}"
