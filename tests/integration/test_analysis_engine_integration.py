...
class TestAnalysisEngineRefactoredIntegration:
    ...
    def minimal_config(self):
        ...
        config.config["external_tools"] = {"apktool_enabled": False, "jadx_enabled": False}
        return config
    ...
    def test_complete_context_setup_to_cleanup_workflow(self, analysis_engine, valid_apk_path):
        ...
        with patch("dexray_insight.core.analysis_engine.TemporalDirectoryManager") as mock_tm_class:
            ...
            context = analysis_engine._setup_analysis_context(
                valid_apk_path, androguard_obj=None, timestamp="integration_test"
            )
            ...
            with patch.object(analysis_engine, "_execute_analysis_modules") as mock_execute_modules:
                ...
                pipeline_results = analysis_engine._execute_analysis_pipeline(context, ["test_module"])
            ...
            mock_temporal_manager.create_temporal_directory.assert_called_once_with(valid_apk_path, "integration_test")
    ...
    def test_temporal_directory_lifecycle_integration(self, analysis_engine, valid_apk_path):
        ...
        with patch("dexray_insight.core.analysis_engine.TemporalDirectoryManager") as mock_tm_class:
            ...
            context = analysis_engine._setup_analysis_context(valid_apk_path, timestamp="lifecycle_test")
            ...
            analysis_engine._handle_analysis_cleanup(context.temporal_paths, preserve_on_error=False)

            # Verify cleanup was called
            mock_temporal_manager.cleanup_temporal_directory.assert_called_once_with(mock_temporal_paths, force=True)

            # Assert: Complete lifecycle verification
            mock_temporal_manager.create_temporal_directory.assert_called_once_with(valid_apk_path, "lifecycle_test")
    ...
    def test_refactored_functions_maintain_original_analyze_apk_behavior(self, analysis_engine, valid_apk_path):
        ...
        with patch("dexray_insight.core.analysis_engine.TemporalDirectoryManager") as mock_tm_class, patch.object(
            analysis_engine, "_execute_external_tools"
        ) as mock_external_tools, patch.object(analysis_engine, "_create_full_results") as mock_create_results:
            ...
            mock_temporal_manager.create_temporal_directory.return_value = mock_temporal_paths
            mock_temporal_manager.check_tool_availability.return_value = True
            mock_temporal_manager.process_apk_with_tools.return_value = {"unzip": True, "jadx": True, "apktool": True}
