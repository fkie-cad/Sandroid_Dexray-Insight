...
class RealAPKTestRunner:
    ...
    def build_pytest_command(self, args, env_info):
        ...
        if args.ci_only or env_info["is_ci"]:
            ...
            test_dirs.extend(
                ["tests/integration/test_real_apk_ci_safe.py", "tests/integration/test_real_apk_local_dev.py"]
            )
        else:
            test_dirs.extend(
                [
...
Examples:
  ...
  python tests/run_real_apk_tests.py --ci-only --create-baseline
    ...
    parser.add_argument("--ci-only", action="store_true", help="Run only CI-safe tests (exampleapp-release.apk)")
    parser.add_argument("--local-dev", action="store_true", help="Run all tests including malware samples")
    ...
    parser.add_argument("--no-external-api", action="store_true", help="Mock all external API calls")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    parser.add_argument("--parallel", action="store_true", help="Run tests in parallel")
    parser.add_argument("--create-baseline", action="store_true", help="Create new baseline results")
