...
def _resolve_vt_config(config):
    """Resolve the VirusTotal API key and enabled flag from configuration."""
    if config is None:
...
def _build_vt_result_dict(json_response):
    """Build the VirusTotal result dictionary from a successful API response."""
...
def vt_check_file_reputation(hash_value, config=None):
    ...
    api_key, enabled = _resolve_vt_config(config)
    ...
    if response.status_code == 200:
        ...
        result_dict = _build_vt_result_dict(json_response)
